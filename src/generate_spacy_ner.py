...
if __name__ == '__main__':
    ...
    for i,series in data_all.iterrows():
        if series['question1'] not in q_dict:
            q_dict[series['question1']] = series['question1']
        if series['question2'] not in q_dict:
            q_dict[series['question2']] = series['question2']

    print "clearn str"
    q_clean_dict = {}
    for k in tqdm(q_dict):
        q_clean_dict[k] = clean_string(q_dict[k])
        # print q_dict[k], q_clean_dict[k]

    print "spacy str -> obj"
    spacy_obj_dict = {}
    for k in tqdm(q_clean_dict):            
        if not q_clean_dict[k]:
            q_clean_dict[k] = ''
        spacy_obj_dict[k] = NLP(unicode(q_clean_dict[k]))
        # print q_clean_dict[k], spacy_obj_dict[k]
    ...
    print "construct vote dictionary"
    ...
    processed_dic = {}
    for qid in tqdm(spacy_obj_dict):
        processed_dic[qid] = ' '.join(process_question_with_spacy(spacy_obj_dict[qid]))
        # print spacy_obj_dict[qid] 
        # print "=> " + processed_dic[qid]

    train['question1_porter'] = train['question1'].apply(lambda x:processed_dic[x])
    test['question1_porter'] = test['question1'].apply(lambda x:processed_dic[x])

    train['question2_porter'] = train['question2'].apply(lambda x:processed_dic[x])
    test['question2_porter'] = test['question2'].apply(lambda x:processed_dic[x])

    train.to_csv(args.train_output_file)
    test.to_csv(args.test_output_file)

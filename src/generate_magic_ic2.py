...
if __name__ == '__main__':
    ...
    g = nx.Graph()
    g.add_nodes_from(df.question1_porter)
    g.add_nodes_from(df.question2_porter)
    edges = list(df[['question1_porter', 'question2_porter']].to_records(index=False))
    g.add_edges_from(edges)


    def get_intersection_count(row):
        return(len(set(g.neighbors(row.question1_porter)).intersection(set(g.neighbors(row.question2_porter)))))

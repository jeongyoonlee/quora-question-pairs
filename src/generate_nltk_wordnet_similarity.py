...
def similarity(row):
    ...
    question1 = filter(lambda x: x in printable, question1)
    question2 = filter(lambda x: x in printable, question2)
    sim = DELTA * semantic_similarity(question1, question2, True) + \
           (1.0 - DELTA) * word_order_similarity(question1, question2)

    print row[4], row[5], sim
    return sim
...
if __name__ == '__main__':
    ...
    train['wordnet_similarity'] = train.apply(similarity, axis=1, raw=True)
    test['wordnet_similarity'] = test.apply(similarity, axis=1, raw=True)
    feats.append('wordnet_similarity')
